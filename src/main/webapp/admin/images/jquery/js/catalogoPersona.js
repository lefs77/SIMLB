...
function consultarOp(op){
			...
			if(typeof op.value !="undefined" && op.value!="0"){
				...
				if(op.value=="1"){					
					...
					//$("#tablaPersona tr").remove();
					$("#tablaPersona tr td").empty();
	...
	function registrarPersonaViaAjax() {
		...
		if( $("#nombre-form").val().length < 1 || $("#apellido-form").val().length < 1 ||
			...
			$("#codigoPostal-form").val().length < 1 || $("#municipio-form").val().length < 1 || $("#colonia-form").val().length < 1 ){
				...
				//Muestra la alerta de telefonos incorrectos
				document.getElementById("alerta-form-danger-telefono").style.display='none';
			...
			if($("#telefono-form").val().length < 10 ){
				
				//Muestra la alerta de telefonos incorrectos
				document.getElementById("alerta-form-danger-telefono").style.display='block';
				//Muestra la alerta de campos vacios
