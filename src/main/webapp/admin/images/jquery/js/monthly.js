	...
	$.fn.extend({
		...
		function setMonthly(m, y){
			...
			if (setMonth == currentMonth && setYear == currentYear) {
				...
				$('#' + uniqueId + ' .monthly-header-title').html('<a href="#" class="monthly-header-title-date"  onclick="return false">'+monthNames[m - 1] +' '+ y +'</a><a href="#" class="monthly-reset" title="Set to today"> HOY </a> ');
			}
...
//						alert(eventId+", "+eventTitle+", "+startTime+", "+startPeriod+", "+endTime);
//						alert(currentDay+"/ "+setMonth+" /"+setYear);
				...
				var eventsResource = options.url;//(options.dataType == 'json' ? options.xmlUrl : options.url);
				
				//alert(eventsResource);
				...
				var cargarRerservacionesForm = {};
				cargarRerservacionesForm["username"] = "lalo";
				cargarRerservacionesForm["email"] = "lefws";
				...
				$.ajax({
					...
					url : "cargarRerservaciones",
					data : {cargarRerservaciones:JSON.stringify(cargarRerservacionesForm)},
					dataType : 'json',
					timeout : 100000,
		...
		function viewToggleButton(){
			if($('#'+uniqueId+' .monthly-event-list').is(":visible")) {
				...
				$('#'+uniqueId+' .monthly-header-title').prepend('<a href="#" class="monthly-cal" title="Back To Month View"> MES </a>');
			}
		}
